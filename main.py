...
FRONTEND_DIR = Path(**file**).resolve().parent
...
1. If the PDF itself mentions an abbreviation (e.g., the “Fund”, the “Partnership”), use the more specific market-known name instead.
1. Keep it concise (under 25 chars) but uniquely identifiable.
...
1. Remove ALL legal entity suffixes and their preceding punctuation: “, L.P.”, “ LP”, “, LLC”, “ Ltd.”, “, SCSp”, “, SCA”, “ Inc.”, “ Corp.”, “ Limited”, “, L.L.C.”
   Remove the comma/space before the suffix too: “ACIP Parallel Fund A, L.P.” → remove “, L.P.” → “ACIP Parallel Fund A”
1. Remove parenthetical clarifications: (the “XX Fund”), (“Parallel Fund”), (collectively, the “Combined Fund”)
1. KEEP critical naming words: Partners, Fund, Trust, Investment, Capital — these distinguish different funds.
1. Convert Roman numerals to Arabic: I→1, II→2, III→3, IV→4, V→5, VI→6, VII→7, VIII→8, IX→9, X→10, etc.
1. Lowercase, collapse multiple spaces to single space, trim
1. The result must be DETERMINISTIC: the same full fund name must ALWAYS produce the same Fund_ID_Key.
...
1. LP_net_amount: Use the “Total Contributions” or “Total Distribution” column value
   from the row matching Investor ID {lp_code}. This is the ONLY amount that matters.
1. line_items: Extract each column amount from the Investor ID {lp_code} row as separate
...
1. Commitment fields: Use “Beginning Unused Commitment” and “Ending Unused Commitment”
   from the Investor ID {lp_code} row for Unfunded_prior and Unfunded_after.
1. Do NOT use “TOTAL LP”, “Total”, or any summary/subtotal row values.
1. If Investor ID {lp_code} shows a dash “-” or blank for an amount, report 0.
1. LP_Name_full / LP_Name_short: Set to “N/A” (omnibus notices don’t name individual LPs).
1. Fund-level info (fund name, dates, wire instructions): extract from the cover letter pages as usual.
   “””
...
1. LP_code MUST be “{lp_code}”.
1. Match each numeric value above to the corresponding column header, left to right.
...
1. Each intermediate column = one line_item. Use column headers as item_name.
   Prefix with the section name if visible in headers (e.g. “India Grid Trust - Investment Amount”).
1. Numbers in parentheses like (1,234) are NEGATIVE.
1. A dash “-” or blank = 0.
1. LP_Name_full / LP_Name_short: Set to “N/A” (omnibus notices don’t name individual LPs).
1. Fund name, dates, wire instructions: extract from the cover letter (page 1) of the PDF as usual.
1. TRUST THE PRE-EXTRACTED NUMBERS. They are taken directly from the PDF text layer.
   “””
...
1. CONCRETE EXAMPLES of items that MUST be in ONE group:
...
1. Strip ALL transaction-type prefixes to find the underlying asset name. The core asset name is what comes AFTER the prefix.
1. Fee/expense items with the SAME base name but different prefixes are ONE group:
...
1. Choose the SHORTEST clean name (without prefix/suffix) as the canonical group key.
1. Items that genuinely have no related variants remain as singletons.
...
js = raw_text
mt_idx = raw_text.find(”`") if mt_idx >= 0: end_idx = raw_text.find("`”, mt_idx + 3)
if end_idx > mt_idx:
js = raw_text[mt_idx+3:end_idx]
if js.startswith(“json”):
js = js[4:]
